import { Box, Flex, Heading, Image, Text } from "@chakra-ui/react";
import React from "react";
import { useNavigate } from "react-router-dom";

const Popup = () => {
  const navigate = useNavigate();
  return (
    <Box w="100%" bg="rgb(252, 229, 216)" p="20px" position={"absolute"} left="0" right="0" top="100px">
      <Text p="15px">TIME TRACKING FOR EVERY TEAM SIZE</Text>
      <Flex gridGap="20px" p="15px">
        <Flex gridGap="10px" onClick={() => navigate("/track/freelance-time-tracking")}>
          <Box>
            <Image src="data:image/svg+xml;base64," />
          </Box>
          <Box>
            <Heading size="md">For freelancers and solo users</Heading>
            <Text>For solo entrepreneurs with a growing list of clients</Text>
          </Box>
        </Flex>
        <Flex gridGap="10px" onClick={() => navigate("/track/time-tracking-small-teams")}>
          <Box>
            <Image src="data:image/svg+xml;base64," />
          </Box>
          <Box>
            <Heading size="md">For small teams</Heading>
            <Text>
              For teams of less than 10 that need to track projects and bill
              clients
            </Text>
          </Box>
        </Flex>
        <Flex gridGap="10px" onClick={() => navigate("/track/time-tracking-large-teams")}>
          <Box>
            <Image src="data:image/svg+xml;base64," />
          </Box>
          <Box>
            <Heading size="md">For big teams</Heading>
            <Text>
              For growing teams that need robust and flexible solutions for time
              and project tracking
            </Text>
          </Box>
        </Flex>
      </Flex>
    </Box>
  );
};

export default Popup;
